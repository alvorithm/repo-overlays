...
**Source worktree redirect** — `_worktree_redirect()` (config.py) asks git once for the working directory's common git dir and toplevel. A main checkout owns its common dir, so `common_dir.parent == toplevel` returns `None` immediately and nearly every invocation costs that one `rev-parse`; only a caller genuinely inside a linked worktree runs the second command, `git worktree list --porcelain`, whose first record is the main checkout. `load_config` then substitutes the worktree for any source registered at that main checkout and keeps the registered path in `SourceConfig.registered_path`, so `config` and `status` can name both. This is the *source* half of the entry above, which resolves destinations: a worktree of `memory-bus` used to be no source at all, which is why parallel work on an overlay source had to serialise. The redirect is deliberately visible rather than silent, because a source's fixed targets are absolute: applying from a worktree overwrites what the main checkout rendered. `write()` (manifest.py) records `source_paths`, `_already_applied()` treats a manifest naming a different tree as not applied so the next apply elsewhere moves every link back, and `cmd_status` reports the mismatch as `foreign-tree`.
